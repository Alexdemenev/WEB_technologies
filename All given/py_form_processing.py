...
def py_form_processing(db,cur):
    ...
    if (form.getvalue("qs_type") == "drop"):# очищаем файлы и удаляем таблицу из базы
        ...
        cur.execute("""INSERT INTO `form_processing` (`id`, `qs_file_name`, `qs_file_price`, `qs_file_amount`, `qs_file_type`, `qs_file_date`, `qs_file_time`) 
            VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')""" % 
            ('NULL' , form["qs_name"].value, form["qs_price"].value ,form["qs_amount"].value, form["qs_type"].value , time.strftime('%Y-%m-%d'), time.strftime('%H:%M:%S')))
        db.commit()
