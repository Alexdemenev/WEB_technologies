...
def py_sql_shop(cur, qr_string):
    ...
    cur.execute("""SELECT `shop_lev_1` FROM `shop` GROUP BY `shop_lev_1` ORDER BY `shop_lev_1` DESC""")
    shop_lev_1 = cur.fetchall()
    #print("\nshop_lev_1:", shop_lev_1)
    shop_lev_1_current = str(shop_lev_1[0][0]) #при необходимости можно организовать цикл
    #print("\nshop_lev_1_current:", shop_lev_1_current)    
    cur.execute("""SELECT `shop_lev_2` FROM `shop`  WHERE `shop_lev_1` = %s  GROUP BY `shop_lev_2` ORDER BY `shop`.`shop_price` DESC""", shop_lev_1_current)
    shop_lev_2  =  cur.fetchall()
    #print ("\nshop_lev_2:", shop_lev_2)
    for result in shop_lev_2:
        to_qs_result=result[0].replace(" ","_") #решаем проблему строки запроса
